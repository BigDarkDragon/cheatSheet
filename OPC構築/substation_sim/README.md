...
| **Fix #3** | `double free or corruption (fasttop)` | 制御ハンドラからの直接的なデータモデル更新を廃止。フラグベースの遅延更新に変更し、全更新を `lockDataModel/unlockDataModel` 区間内に集約 | `substation_server.c` |

> **重要:** v2 に更新する場合は `make clean && make` で完全リビルドすること。古い `.o` ファイルが残っていると修正が反映されない。
