...
## v3 修正履歴 (2026-07-21) — double free 完全修正

| # | 症状 | 根本原因 | 修正内容 | 対象 |
|---|---|---|---|---|
| **Fix #3 完全** | `double free or corruption (fasttop)` Cogent → KEP → IED で一気通貫時にクラッシュ | ICD の DPC で CO 属性を個別 DA として定義 → libIEC61850 の MMS Read ハンドラが `addComplexValueToResultList()` でバッチ読み取り時にツリー走査を誤る | DPC の CO 属性を `Oper` 構造体にグループ化。DataSet / ReportControl を削除 | `substation.icd` |
| **追加** | 未初期化 MmsValue によるクラッシュ防止 | genmodel が Check (BitString) / Octet64 の初期バッファを正しく確保しない場合がある | 全属性（MMXU 計測値含む）をサーバ起動前に明示的に初期化 | `substation_server.c` |

> **重要:** v3 に更新する場合は **genmodel.jar による再生成が必須**（ICD 構造が変更されたため）:
> ```bash
...
| # | 症状 | 修正内容 | 対象ファイル |
|---|---|---|---|
| **Fix #1** | `OCTET_STRING_64` ビルドエラー | `Octet64` に変更 | `substation.icd` L237 |
| **Fix #2** | セグメンテーションフォルト | `IedServer_updateDbposValue` に DataAttribute (`_Pos_stVal`) を渡すよう修正。DataObject (`_Pos`) を渡していたのが原因 | `substation_server.c` |
| **Fix #3 暫定** | `double free or corruption (fasttop)` | 制御ハンドラからの直接的なデータモデル更新を廃止。フラグベースの遅延更新に変更 → **v3 で完全修正** | `substation_server.c` |

> **重要:** v3 では genmodel の再実行が必要。`make clean && make` だけでは不十分。
