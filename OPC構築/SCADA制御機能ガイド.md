...
>
> **変更点:**
> - IED を 4台（4プロセス）に拡張。各 IED は XCBR1(遮断器) + XSWI1(断路器) + MMXU1(計測) の構成
> - XCBR2 は削除（各 IED が1遮断器+1断路器を担当）
> - 電流シミュレーション改善: 遮断器 AND 断路器が共に Closed の場合のみ電流が流れる
> - タグ名セパレータ: クローズ系 (Windows 7) では `.`、検証環境 (Windows 11) では `$`
>
> **ポート割り当て:**
>
> | IED | ポート | フィーダー | KEPserver チャンネル |
> |-----|--------|-----------|-------------------|
...
>
> **タグ名の例（クローズ系 `.` 区切り）:**
> - 遮断器状態: `XCBR1.ST.Pos.stVal`
> - 遮断器制御: `XCBR1.CF.Pos.PosCmd`
> - 断路器状態: `XSWI1.ST.Pos.stVal`
> - 断路器制御: `XSWI1.CF.Pos.PosCmd`
> - 電流: `MMXU1.MX.A.phsA.cVal.mag.f`
>
> **SCADA 画面 (HTML):**
> - 4フィーダーの単線結線図を表示
> - 各 IED のドメインプレフィックスを設定バーで個別に設定可能
> - SEP セレクタで `.` / `$` を切り替え可能
...
substation_server      KEPServerEX               DataHub
  │                      │                         │
  │  MMS Write ────────→ │  OPC DA Write ────────→ │
  │  (stVal に直接)      │  (DCOM IOPCSyncIO)     │
  │                      │                         │
  │  stVal=1(Open)       │  Write(stVal, 1)        │  操作員: [Open] ボタン押下
  │                      │                         │
  │  WriteHandler発火    │                         │
...
### 2.2 制御方式の選択（v3.1 検証結果に基づく）

> **⚠ 検証結果:** KEPServerEX v7 の IEC 61850 MMS Client ドライバは、
> OPC DA の Sync Write を **MMS Operate に自動変換しない**。
> `Oper.ctlVal` に Write すると `HR=0x80004005`（MMS Write 拒否）で失敗する。
>
> このため、本環境では **`stVal` への直接 Write** で制御を行う。

```
                    IEC 61850 正規         本環境（v3.1）         Industroyer
                    ─────────────         ──────────────         ──────────
書き込み先:         CO$Pos$Oper$ctlVal    ST$Pos$stVal           ST$Pos$stVal
MMS 操作:           Operate サービス       MMS Write              MMS Write (via OPC DA)
KEPServerEX:       Operate に変換         そのまま MMS Write      そのまま MMS Write
                   ★ 動作せず             ★ 動作確認済み          ★ 実際の攻撃と同じ
```

**Industroyer は実際の攻撃でも stVal への直接書き込みを使用した。**
本環境は攻撃再現が目的であるため、この方式がむしろ正確である。

### 2.3 stVal タグの Read/Write 設定

KEPServerEX で XCBR1/XCBR2/XSWI1 の stVal タグを書き込み可能にする。

```
KEPServerEX Configuration:
  IEC61850_Ch1 → IED1 を展開
  → 以下のタグを右クリック → [Properties]

  ┌───────────────────────────────────────────────────────────┐
  │  タグ                                 Client Access       │
  │                                                           │
  │  XCBR1$ST$Pos$stVal                   Read/Write ← 変更  │
  │  XCBR2$ST$Pos$stVal                   Read/Write ← 変更  │
  │  XSWI1$ST$Pos$stVal                   Read/Write ← 変更  │
  │                                                           │
  │  ※ その他の stVal (Beh, Health 等) は Read Only のまま    │
  └───────────────────────────────────────────────────────────┘
```

> **タグが自動生成されていない場合は手動追加:**
> ```
> IED1 → 右クリック → [New Tag]
>   Tag Name:     XCBR1_Pos_stVal
>   Address:      IED1CBIED/XCBR1$ST$Pos$stVal
>   Data Type:    Byte
...
Quick Client:
  ...
  XCBR1$ST$Pos$stVal を右クリック → [Sync Write]
  → Data type: Byte (または Char)
  → Value: 1         ← Dbpos: 1=Open, 2=Closed
  → [OK]

  → IED のコンソールに以下が表示されること:
    [WRITE] XCBR1 stVal=1 (Open) from 192.168.56.20 → queued
    [UPDATE] XCBR1: state=1 (Open), OpCnt=1

  → MMXU1 の電流値が ~250A → 0A に変化すること

  === 遮断器を復旧する ===

  XCBR1$ST$Pos$stVal を右クリック → [Sync Write]
  → Value: 2         ← Dbpos: 2=Closed
  → [OK]

  → IED のコンソール:
    [WRITE] XCBR1 stVal=2 (Close) from 192.168.56.20 → queued
    [UPDATE] XCBR1: state=2 (Closed), OpCnt=2

  → 電流値が 0A → ~250A に復旧

  ※ CO$Pos$Oper$ctlVal への Sync Write は使用しない（MMS Write 拒否のため）
```
...
DataHub Manager:
  ...
  タグツリーを展開:
    ...
    └── IEC61850_Ch1
        └── IED1
            └── IED1CBIED
                ├── XCBR1$ST$Pos$stVal    = 2 (Closed)  ← ★ ここに Write
                ├── XCBR2$ST$Pos$stVal    = 2 (Closed)
                ├── XSWI1$ST$Pos$stVal    = 2 (Closed)
                └── ...

  === 遮断器を開放する操作 ===

  XCBR1$ST$Pos$stVal を右クリック → [Write Value...]
  → Value: 1  (Dbpos: 1 = Open)
  → [Write]
  ...
  ※ CO$Pos$Oper$ctlVal は使用しない（MMS Write 拒否のため）
```
